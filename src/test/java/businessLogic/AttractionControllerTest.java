...
class AttractionControllerTest {
    ...
    void When_AddingAttraction_With_OccupiedEmployee_Expect_IllegalArgumentException() throws Exception {
        ...
        // Add Attraction
        attractionsController.addAttraction("Pressure", 10, 5, LocalDateTime.now().plusHours(1), LocalDateTime.now().plusHours(2), "MRNSML1","Pressure");
        // Add non overlapping Attraction with same employee
        attractionsController.addAttraction("Supremacy", 10, 2, LocalDateTime.now().plusHours(5), LocalDateTime.now().plusHours(6), "MRNSML1","Supremacy");

        // Add overlapping Attraction with same employee
        Assertions.assertThrows(
                IllegalArgumentException.class,
                () -> attractionsController.addAttraction("Uprising", 10, 8, LocalDateTime.now().plusHours(2), LocalDateTime.now().plusHours(14), "MRNSML1","Uprising"),
